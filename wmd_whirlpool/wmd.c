...
// general :
 ...
 const unsigned char * message = (const unsigned char *) data;  // originally: const char * message == data;
 struct /* sWMD */ {
   unsigned char SbitLength[ 32 ];
   unsigned char Sbuffer   [ 64 ];
   int SbufferBits;
   int SbufferPos;
   unsigned long long Shash[ 8 ];
 } w;
 ...
 const unsigned long long C0[ 256 ] = {
 ...
 const unsigned long long C1[ 256 ] = {
 ...
 const unsigned long long C2[ 256 ] = {
 ...
 const unsigned long long C3[ 256 ] = {
 ...
 const unsigned long long C4[ 256 ] = {
 ...
 const unsigned long long C5[ 256 ] = {
 ...
 const unsigned long long C6[ 256 ] = {
 ...
 const unsigned long long C7[ 256 ] = {
...
/*  P R O G R A M  */
  ...
  (void) memset( w.SbitLength, 0, 32                );
  if( ~ (uint32_t) 0 == datalength ) datalength = (uint32_t) strlen( data );
  w.SbufferBits = w.SbufferPos = 0;
  w.Sbuffer[ 0 ] = 0;
  for( i = 0; i < 8; ++i ) w.Shash[ i ] = 0L;

  // wmd_add :
  sourceBits = ( (unsigned long) datalength ) << 3;
  sourcePos  = 0;
  sourceGap  = (8 - ((int)sourceBits & 7)) & 7;
  bufferRem  = w.SbufferBits & 7;
  buffer     = w.Sbuffer;
  bitLength  = w.SbitLength;
  bufferBits = w.SbufferBits;
  bufferPos  = w.SbufferPos;
  value      = sourceBits;
  ...
  while( sourceBits > 8 ) {
    b = (( message[ sourcePos ] << sourceGap ) & 0xff ) |
        (( message[ sourcePos + 1 ] & 0xff ) >> ( 8 - sourceGap ));
...
WMD_PROCBUF_OUT_1:  ;
  ...
  if( bufferRem + sourceBits < 8 ) bufferBits += sourceBits;
  else {
    bufferPos++;
    bufferBits += 8 - bufferRem;
...
WMD_PROCBUF_OUT_2:  ;
  ...
  w.SbufferBits = bufferBits;
  w.SbufferPos  = bufferPos;

  // wmd_finalize :
  finbuffer     = w.Sbuffer;
  finbitLength  = w.SbitLength;
  finbufferBits = w.SbufferBits;
  finbufferPos  = w.SbufferPos;
  findigest     = digest;

  finbuffer[ finbufferPos ] |= 0x80U >> ( finbufferBits & 7 );
  finbufferPos++;
  if( finbufferPos > 32 ) {
    if( finbufferPos < 64 ) (void) memset( &finbuffer[ finbufferPos ], 0, 64 - finbufferPos );
...
WMD_PROCBUF_OUT_3:  ;
  ...
  if( finbufferPos < 32 ) (void) memset( &finbuffer[ finbufferPos ], 0, 32 - finbufferPos );
  finbufferPos = 32;
  memcpy( &finbuffer[ 32 ], finbitLength, 32);
...
WMD_PROCBUF_OUT_4:  ;
  ...
  for( i = 0; i < 8; ++i ) {
    ...
    findigest[ 1 ] = (unsigned char) ( w.Shash[ i ] >> 48 );
    findigest[ 2 ] = (unsigned char) ( w.Shash[ i ] >> 40 );
    findigest[ 3 ] = (unsigned char) ( w.Shash[ i ] >> 32 );
    findigest[ 4 ] = (unsigned char) ( w.Shash[ i ] >> 24 );
    findigest[ 5 ] = (unsigned char) ( w.Shash[ i ] >> 16 );
    findigest[ 6 ] = (unsigned char) ( w.Shash[ i ] >>  8 );
    findigest[ 7 ] = (unsigned char) ( w.Shash[ i ]       );
    findigest += 8;
  }
  w.SbufferBits = finbufferBits;
...
WMD_PROCBUF_IN:  ;
  ...
  for( i = 0; i < 8; ++i, procbuffer += 8 ) {
    ...
    block[ i ] = \
    (((unsigned long long) procbuffer[ 0 ]         ) << 56 ) ^
    (((unsigned long long) procbuffer[ 1 ] & 0xffL ) << 48 ) ^
    (((unsigned long long) procbuffer[ 2 ] & 0xffL ) << 40 ) ^
    (((unsigned long long) procbuffer[ 3 ] & 0xffL ) << 32 ) ^
    (((unsigned long long) procbuffer[ 4 ] & 0xffL ) << 24 ) ^
    (((unsigned long long) procbuffer[ 5 ] & 0xffL ) << 16 ) ^
    (((unsigned long long) procbuffer[ 6 ] & 0xffL ) <<  8 ) ^
    (((unsigned long long) procbuffer[ 7 ] & 0xffL )       );
  }

  state[ 0 ] = block[ 0 ] ^ ( K[ 0 ] = w.Shash[ 0 ] );
  state[ 1 ] = block[ 1 ] ^ ( K[ 1 ] = w.Shash[ 1 ] );
  state[ 2 ] = block[ 2 ] ^ ( K[ 2 ] = w.Shash[ 2 ] );
  state[ 3 ] = block[ 3 ] ^ ( K[ 3 ] = w.Shash[ 3 ] );
  state[ 4 ] = block[ 4 ] ^ ( K[ 4 ] = w.Shash[ 4 ] );
  state[ 5 ] = block[ 5 ] ^ ( K[ 5 ] = w.Shash[ 5 ] );
  state[ 6 ] = block[ 6 ] ^ ( K[ 6 ] = w.Shash[ 6 ] );
  state[ 7 ] = block[ 7 ] ^ ( K[ 7 ] = w.Shash[ 7 ] );
  ...
  w.Shash[0] ^= state[0] ^ block[0];
  w.Shash[1] ^= state[1] ^ block[1];
  w.Shash[2] ^= state[2] ^ block[2];
  w.Shash[3] ^= state[3] ^ block[3];
  w.Shash[4] ^= state[4] ^ block[4];
  w.Shash[5] ^= state[5] ^ block[5];
